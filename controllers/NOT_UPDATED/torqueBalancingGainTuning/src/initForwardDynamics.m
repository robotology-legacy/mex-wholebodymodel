...
[x_bInit,w_R_bInit] = wbm_getWorldFrameFromFixLnk(CONFIG.constraintLinkNames{1},qjInit);
...
%% Remove robot specific paths to avoid confilcts when restarting the integrator 
rmpath(CONFIG.utility_root);
rmpath(CONFIG.robot_root);
rmpath(CONFIG.plots_root);
rmpath(CONFIG.src_root);
rmpath(CONFIG.gains_root);
rmpath(CONFIG.ikin_root);
