...
Kp_cartesian     = [gainsPCoM zeros(3); zeros(3) eye(3)*gainPhi];
...
Kd_cartesian     = [gainsDCoM zeros(3); zeros(3) eye(3)*gainMomentum];
...
% Tests
% old_KS          = KS; 
% new_KS          = Ap_cartesian_tot*Kp_cartesian*Bp_cartesian_tot + Ap_null_tot* diag(impedances)*Bp_null_tot;
% 
% old_KD          = KD; 
% new_KD          = Ad_cartesian_tot*Kd_cartesian*Bd_cartesian_tot + Ad_null_tot* diag(dampings)*Bd_null_tot;
% 
% old_KS-new_KS
% old_KD-new_KD
...
% Setup the Kronecker product
param.ndof = ndof;

% desired position gains
[Kpx,Kpn] = kron_prod(Ap_cartesian_tot,Bp_cartesian_tot,Ap_null_tot,Bp_null_tot,KSdes,param);
[Kdx,Kdn] = kron_prod(Ad_cartesian_tot,Bd_cartesian_tot,Ad_null_tot,Bd_null_tot,KDdes,param);
...
disp('eigenvalues of the new state matrix')
disp(eig(A_state_new))
