function [X, Y, Z] = solidCylinder(varargin)
    ...
    switch nargin
        case 3
            ...
            [xx, yy, zz] = WBM.utilities.vb.solidRing(varargin{1,1}, ri0, varargin{1,2:3}, 'rect');
        case 2
            if isscalar(varargin{1,1})
                % r = varargin{1}
                % n = varargin{2}
                [xx, yy, zz] = WBM.utilities.vb.solidRing(ri0, varargin{1,1:2}, 'rect');
            else
                % hax = varargin{1}
                % r   = varargin{2}
                [xx, yy, zz] = WBM.utilities.vb.solidRing(varargin{1,1}, ri0, varargin{1,2}, 'rect');
            end
        case 1
            % r = varargin{1}
            [xx, yy, zz] = WBM.utilities.vb.solidRing(ri0, varargin{1,1}, 'rect');
        case 0
            % use the unit radius (default)
            [xx, yy, zz] = WBM.utilities.vb.solidRing(ri0, 1, 'rect');
    end
