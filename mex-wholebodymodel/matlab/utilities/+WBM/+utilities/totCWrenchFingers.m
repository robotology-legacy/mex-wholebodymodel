function wc_tot = totCWrenchFingers(f_cp, a_R_c, a_p_c, varargin)
    %% Total contact wrench applyed by the robot hand on the grasped object.
    %
    %  Sources:
    ...
    %   [2]  Grasp quality measures: review and performance, M. A. Roa & R. Suárez, Springer, Autonomous Robots, Volume 38, Issue 1,
    %        Springer, 2015, <https://link.springer.com/article/10.1007/s10514-014-9402-3>, p. 71, eq. (20) & (22).
    %   [3]  Planning Optimal Grasps, C. Ferrari & J. Canny, Proceedings 1992 IEEE International Conference on Robotics and Automation (ICRA), 1992,
    ...
    if (n_c >= 1)
        switch nargin
            case 6
                ...
                % gamma_s = varargin{2}
                for i = 1:n_c
                    w_c    = WBM.utilities.ctcwrench(f_cp{1,i}, a_R_c, a_p_c, varargin{1:2});
                    wc_tot = wc_tot + w_c;
                end
            case 5
                ...
                % mu_s = varargin{1}
                for i = 1:n_c
                    w_c    = WBM.utilities.ctcwrench(f_cp{1,i}, a_R_c, a_p_c, varargin{1,1});
                    wc_tot = wc_tot + w_c;
                end
            case 4
                % frictionless contact model:
                for i = 1:n_c
                    w_c    = WBM.utilities.ctcwrench(f_cp{1,i}, a_R_c, a_p_c);
                    wc_tot = wc_tot + w_c;
                end
            otherwise
                error('totCWrenchFingers: %s', WBM.wbmErrorMsg.WRONG_ARG);
        end
    end
end
