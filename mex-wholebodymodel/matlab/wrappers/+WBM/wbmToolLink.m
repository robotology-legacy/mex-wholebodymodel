...
classdef wbmToolLink
    ...
    %                                  (default name: *'none'*).
    %   ee_vqT_tt    (double, vector): (7 x 1) VQ-transformation (frame) of the
    %                                  tool relative from the tool-tip (tt) to
    %                                  the link frame of the end-effector (ee).
    %                                  Default vector: :math:`[0, 0, 0, 1, 0, 0, 0]^T`.
    %   pl_idx        (uint8, scalar): Index number of a specified *payload link
    %                                  object* for the case, when a grabbed
