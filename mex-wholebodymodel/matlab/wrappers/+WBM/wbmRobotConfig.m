...
classdef wbmRobotConfig < handle
    ...
    %   ccstr_link_names (cellstr, vector): :math:`(1 \times n_{cstrs})` string array of
    %                                       *contact constraint link names* to specify
    %                                       which link of the robot is in contact with
    %                                       the ground or an object.
    %   nPlds              (uint8, scalar): Number of payloads that are assigned
    %                                       to specific links of the robot (default: 0).
    %   payload_links (:class:`~WBM.wbmPayloadLink`, *vector*): :math:`(1 \times n_{plds})` array of *payload
    %                                                           link objects*, where each payload is assigned
    %                                                           to a specific link of the robot (default: *empty*).
    %
    %                                                           **Note:** The indices 1 and 2 of the array are
    ...
    %                                       number is 0.
    %   tool_links (:class:`~WBM.wbmToolLink`, *vector*): :math:`(n \times 1)` array of *tool link objects*,
    %                                                     where each tool is assigned to a specific link of
    %                                                     the robot. The length :math:`n` of the array can
    ...
    %                                       :attr:`init_state_params` is empty [#f7]_.
