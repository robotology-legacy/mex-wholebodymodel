...
function dstvChi = forwardDynamicsNFBPL(obj, t, stvChi, fhTrqControl, fhTotCWrench, hand_conf, f_cp)
    % Computes the *forward dynamics* of the *whole body model* of a humanoid
    % robot with *payload forces* that are acting on the hands of the robot,
    % but without a *floating base* (*NFBPL*) or by exclusion of it.
    %
    ...
    %
    % See Also:
    %   :meth:`WBM.forwardDynamicsNFB` and :meth:`WBM.forwardDynamicsNFBEF`.
    %
    % References:
    ...
    dnu = jointAccelerationsNFBPL(obj, hand_conf, tau, fhTotCWrench, f_cp, M, c_qv, stp.dq_j);
