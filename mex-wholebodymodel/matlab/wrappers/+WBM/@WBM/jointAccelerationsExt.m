function [ddq_j, acc_data] = jointAccelerationsExt(obj, varargin)
    switch nargin
        case 9 % optimized mode:
            ...
            % get the initial transformation vectors (veT, position-vector with Euler-angles) of the feet:
            fk.init.veT.l_foot = foot_conf.veT_init.l_sole;
            fk.init.veT.r_foot = foot_conf.veT_init.r_sole;

            % calculate the new positions and orientations (VQ-Transformations) for the feet:
            fk.new.vqT.l_sole = mexWholeBodyModel('forward-kinematics', 'l_sole');
            fk.new.vqT.r_sole = mexWholeBodyModel('forward-kinematics', 'r_sole');
        case 8 % normal mode:
            % wf_R_b     = varargin{1}
            wf_p_b       = varargin{1,2};
            q_j          = varargin{1,3};
            ...
            fk.new.vqT.l_sole = mexWholeBodyModel('forward-kinematics', wf_R_b_arr, wf_p_b, q_j, 'l_sole');
            fk.new.vqT.r_sole = mexWholeBodyModel('forward-kinematics', wf_R_b_arr, wf_p_b, q_j, 'r_sole');
    ...
    % extract from the VQ-Transformations of the feet the position-vectors and Euler-angles:
    [p_lfoot, eul_lfoot] = WBM.utilities.frame2posEul(fk.new.vqT.l_sole);
    [p_rfoot, eul_rfoot] = WBM.utilities.frame2posEul(fk.new.vqT.r_sole);
    % create new transformation vectors (veT) for the feet:
    fk.new.veT.l_foot = vertcat(p_lfoot, eul_lfoot);
    fk.new.veT.r_foot = vertcat(p_rfoot, eul_rfoot);
    ...
    if (foot_conf.ground.left && foot_conf.ground.right)
        % error       =           curr. position    -   desired position
        veT_feet_diff = vertcat( (fk.new.veT.l_foot - fk.init.veT.l_foot), ...
                                 (fk.new.veT.r_foot - fk.init.veT.r_foot) );

    elseif (~foot_conf.ground.left && foot_conf.ground.right)
        veT_feet_diff = (fk.new.veT.r_foot - fk.init.veT.r_foot);
    ...
    f_c = -(JcMinvJct \ (JcMinv*(C_qv - tau_gen) - dJcdq - k_v.*(Jc*nu) - k_p.*veT_feet_diff));
