...
function dstvChi = forwardDynamicsFHPCEF(obj, t, stvChi, fhTrqControl, foot_conf, hand_conf, fe_h, ac_h, ac_f)
    % Computes the *forward dynamics* of the *whole body model* of the given
    % floating-base robot with *foot* and *hand pose corrections* and *external
    % forces* (*FHPCEF*) that are acting on the hands of the robot -- *experimental*.
    %
    ...
    %   satisfying the equation :eq:`forward_dynamics_chi_hpcef`.
    %
    % See Also:
    %   :meth:`WBM.forwardDynamicsFHPCPL`.
    %
    % References:
    ...
    dnu = jointAccelerationsFHPCEF(obj, foot_conf, hand_conf, tau, fe_h, ac_h, ...
