...
function dstvChi = forwardDynamicsEF(obj, t, stvChi, fhTrqControl, foot_conf, clnk_conf, fe_c, ac, ac_f)
    % Computes the *forward dynamics* of the *whole body model* of the given
    % floating-base robot with *external forces* (*EF*) that are acting on
    % the contact links of the robot -- *experimental*.
    %
    ...
    %
    %   .. math::
    %      :label: forward_dynamics_chi_ef
    %
    %      \dot{\chi} = \mathrm{FD}(model, t, \chi, \tau, f^{\small C}_e)\:,
    %
    ...
    % of a given time interval, :math:`f^{\small C}_e` are the external forces
    % in contact space :math:`\mathrm{C}` and the vector variable :math:`\chi`
    % to be integrated (compare also with :cite:`Featherstone2008`, p. 41).
    %
    ...
    %                                   for the *foot contact points*.
    %
    % The variable :math:`k` indicates the *size* of the given *force* and *acceleration
    % vectors* in dependency of the specified contact links and feet:
    %
    ...
    %   satisfying the above equation :eq:`forward_dynamics_chi_ef`.
    %
    % See Also:
    %   :meth:`WBM.forwardDynamics` and :meth:`WBM.forwardDynamicsPL`.
    %
    % References:
    ...
    dnu = jointAccelerationsEF(obj, foot_conf, clnk_conf, tau, fe_c, ac, ac_f, stp.dq_j); % optimized mode
