...
function dstvChi = forwardDynamicsFHPCPL(obj, t, stvChi, fhTrqControl, fhTotCWrench, foot_conf, hand_conf, f_cp, ac_f)
    % Computes the *forward dynamics* of the *whole body model* of a humanoid
    % robot with *foot* and *hand pose corrections* and *payload forces*
    % (*FHPCPL*) that are acting on the hands of the robot -- *experimental*.
    %
    ...
    %
    % The variable :math:`k` indicates the *size* of the given *acceleration
    % vectors* in dependency of the specified feet and hands:
    %
    ...
    %   satisfying the equation :eq:`forward_dynamics_chi_pl`.
    %
    % See Also:
    %   :meth:`WBM.forwardDynamicsFHPCEF`.
    %
    % References:
    ...
    dnu = jointAccelerationsFHPCPL(obj, foot_conf, hand_conf, tau, fhTotCWrench, f_cp, ...
