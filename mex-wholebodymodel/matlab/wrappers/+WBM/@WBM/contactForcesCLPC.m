function [f_c, tau_gen] = contactForcesCLPC(obj, clink_conf, tau, f_e, a_c, Jc, djcdq, M, c_qv, varargin)
    switch nargin
        ...
        case 14 % normal modes:
            ...
            error_lnk = clnkPoseError(obj, clink_conf, varargin{1:3});
        case 13
            % general case:
            nu = varargin{1,4};
        ...
        case 11 % optimized modes:
            ...
            error_lnk = clnkPoseError(obj, clink_conf);
        case 10
            % general case:
            nu = varargin{1,1};
...
function error_lnk = clnkPoseError(obj, clink_conf, varargin)
    ...
    if (ctc_l && ctc_r)
        ...
        % get the error (distances) between the link poses:
    ...
    elseif ctc_r
        ...
        % both links are not in contact with the ground/object ...
        error('WBM::contactForcesCLPC: %s', WBM.wbmErrorMsg.NO_LNK_IN_CTC); % only temporary --> f_c must be set to zero --> check dim!
    end
end
...
function fk_new_pose = clnkPoseTransformationRight(clink_r, varargin)
    ...
    switch nargin
        case 4 % normal mode:
            ...
            % wf_p_b     = varargin{2}
            % q_j        = varargin{3}
            fk_new_pose.vqT_llnk = mexWholeBodyModel('forward-kinematics', varargin{1,1}, varargin{1,2}, varargin{1,3}, clink_r);
        case 1 % optimized mode:
            fk_new_pose.vqT_llnk = mexWholeBodyModel('forward-kinematics', clink_r);
        otherwise
